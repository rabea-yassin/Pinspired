...
export function Profileliked() {
  let history = useHistory();

  const [msg, setmsg] = useState('');
  const [userinfo, setuserinfo] = useState([]);
  const [uplodsyouliked, setuplodsyouliked] = useState([]);
  ...
  useEffect(() => {
    Axios.get(`http://localhost:3001/upload/liked/${sessionStorage.getItem("userid")}`).then((response) => {
      setuplodsyouliked(response.data);
      console.log("liked uplouds fetched")
    });
  }, []);
      ...
      {sessionStorage.getItem("loggedin") == "true" ?
        <div>
          ...
          <div className="viewingcateg">
            <button className="profbtn " onClick={() => { history.push("/profile"); }}> my Uploads </button>
            <button className="profbtn " onClick={() => { history.push("/Profile/saved"); }}> saved </button>
            <button className="profbtn selected" onClick={() => {history.push("/Profile/liked"); }}> liked </button>

          </div>
          <Gridcontaner  array={uplodsyouliked}></Gridcontaner>
        </div>
